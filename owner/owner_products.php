...
if (isset($_POST['add_product'])) {
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($mime, $allowed)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $uploadDir . $filename)) {
                    $imageFilename = 'assets/storeproducts/' . $filename;   // stored with relative path
                }
            } else {
                $message = '<div class="alert error">Invalid image type (only JPG, PNG, WebP, GIF).</div>';
            }
        } else {
            $message = '<div class="alert error">Upload error (code ' . $file['error'] . ').</div>';
        }
    }
    ...
    if (empty($message)) {
        ...
        $sql = "INSERT INTO store_products (store_id, category_id, name, price, image)
                VALUES ($store_id, $cat_id, '$name', $price, '$img')";
        if (mysqli_query($conn, $sql)) {
            $message = '<div class="alert success">✅ Product added.</div>';
        } else {
...
// Edit an existing product (via POST)
if (isset($_POST['edit_product'])) {
    ...
    // update other fields
    $sql = "UPDATE store_products SET name='$name', price=$price, category_id=$cat_id WHERE id=$prod_id AND store_id=$store_id";
    if (mysqli_query($conn, $sql)) {
...
if (isset($_GET['delete'])) {
    ...
    // remove image file
    $img = mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM store_products WHERE id=$del_id AND store_id=$store_id"));
    if ($img && !empty($img['image'])) {
        $filepath = '../' . $img['image'];
        if (file_exists($filepath)) unlink($filepath);
    }
    mysqli_query($conn, "DELETE FROM store_products WHERE id=$del_id AND store_id=$store_id");
    $message = '<div class="alert error">🗑️ Product deleted.</div>';
}
...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        body { font-family:'Outfit',sans-serif; background:var(--bg); color:var(--foreground); padding:20px; min-height:100vh; }
        ...
        .btn { display:inline-block; padding:10px 20px; border-radius:30px; background:var(--primary); color:#052e24; font-weight:700; border:none; cursor:pointer; transition:0.2s; font-size:14px; }
