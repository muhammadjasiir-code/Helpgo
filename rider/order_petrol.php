...
$order = mysqli_fetch_assoc(mysqli_query($conn, "
    ...
    WHERE o.order_id = '$orderId'
      AND o.service_type = 'petrol'
      AND o.rider_id = (SELECT id FROM riders WHERE user_id = $riderId)
    LIMIT 1
"));

if (!$order) {
    die("<div style='color:#fff;text-align:center;margin-top:50px;font-family:sans-serif;background:#0f1117;min-height:100vh;padding-top:80px;'>Petrol order not found or not assigned to you.<br><br><a href='home.php' style='color:#FF6B35;'>Back to Home</a></div>");
}
...
if (isset($_POST['update_status'])) {
    ...
    if (in_array($newStatus, array('accepted', 'picked_up', 'in_transit'))) {
        ...
        header("Location: order_petrol.php?id=$orderId");
        exit;
    }
}
...
if (isset($_POST['verify_otp'])) {
    ...
    if ($entered_otp === $order['otp']) {
        ...
        $settingPlatformFee = getSetting('platform_fee');
        $platformFee = floatval($settingPlatformFee !== null && $settingPlatformFee !== '' ? $settingPlatformFee : 5);
        $deliveryFare = floatval($order['delivery_fare']);
        $petrolAmount = floatval(isset($order['total_amount']) ? $order['total_amount'] : 0);
        $paymentMethod = strtolower(isset($order['payment_method']) ? $order['payment_method'] : '');

        // Prevent double-processing on repeat submits
        $chk = mysqli_query($conn, "SELECT id FROM wallet_transactions WHERE user_id = $riderId AND description LIKE '%order #$orderId%' LIMIT 1");
        $alreadyProcessed = ($chk && mysqli_num_rows($chk) > 0);

        if (!$alreadyProcessed) {
            if ($paymentMethod === 'cash' || $paymentMethod === 'cod') {
                // COD: rider already collected full cash from customer -> debit ₹5 platform fee
                ...
                // UPI / online: customer paid total (petrol + delivery fare) online.
                // Credit rider = total_amount - platform fee.
                $riderEarning = $petrolAmount - $platformFee;
                mysqli_query($conn, "UPDATE wallet SET balance = balance + $riderEarning WHERE user_id = $riderId");
                mysqli_query($conn, "INSERT INTO wallet_transactions (user_id, transaction_type, amount, description) VALUES ($riderId, 'credit', $riderEarning, 'Earnings for order #$orderId (UPI: total - fee)')");
            }
        }
...
<head>
    ...
    <?php if ($order['payment_method'] == 'upi' && $order['payment_status'] != 'paid' && $order['status'] != 'delivered' && $order['status'] != 'cancelled'): ?>
    <meta http-equiv="refresh" content="2">
    <?php endif; ?>
    <title>Petrol Order #<?= $orderId ?> – HelpGo Rider</title>
...
<div class="container">
    ...
    <div class="card">
        ...
        <?php endif; ?>
        <?php if (isset($order['total_amount']) && $order['total_amount'] > 0): ?>
            <div class="detail-row"><span>Total Amount</span><span style="color:var(--primary);font-weight:600;">₹<?= number_format($order['total_amount'],2) ?></span></div>
        <?php endif; ?>
    </div>
        ...
        <?php if ($order['status'] != 'delivered' && $order['status'] != 'cancelled'): ?>
            ...
            <?php else: ?>
                <?php if ($order['status'] == 'accepted'): ?>
                    <form method="POST"><input type="hidden" name="status" value="picked_up"><button type="submit" name="update_status" class="btn">⛽ Mark as Picked Up (Fuel Loaded)</button></form>
